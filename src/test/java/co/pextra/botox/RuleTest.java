...
public class RuleTest extends CamelTestSupport {
	...
	private StatelessKieSession getStatelessKieSession(Resource resource) {
		return getStatelessKieSessionFromResource(
				resource.setSourcePath("src/main/resources/rule.drl").setResourceType(ResourceType.DRL));
	}
	...
	private static KieSession getKieSession(Resource resource) {
		return getKieSessionFromResource(
				resource.setSourcePath("src/main/resources/my-process.bpmn").setResourceType(ResourceType.BPMN2));
	}
	...
	/*@Test
	public void TestProcessStringDefinition2() {
		KieSession kSession = null;
		try {
			kSession = getKieSessionFromResource(ResourceFactory.newByteArrayResource(process.getBytes("utf-8"))
					.setSourcePath("src/main/resources/my-process.bpmn").setResourceType(ResourceType.BPMN2));
	...
	public void TestProcessStringDefinition() {
		...
		try {
			...
			kfs.write(ResourceFactory.newByteArrayResource(process.getBytes("utf-8"))
					.setSourcePath("src/main/resources/my-process.bpmn").setResourceType(ResourceType.BPMN2));
	...
	public void TestRuleStringDefinition() {
		...
		try {
			...
			kfs.write(ResourceFactory.newByteArrayResource(ruleContent.getBytes("utf-8"))
					.setSourcePath("src/main/resources/my-rule.drl").setResourceType(ResourceType.DRL));
	...
	public void Test() throws InterruptedException, ExecutionException {
		...
		try {
			...
			LOG.info("Loading String Process Definition");
			...
			//kfs.write(ResourceFactory.newFileResource("src/main/resources/BotoxDrugsScenario.drl"));

			KieBuilder kieBuilder = kieServices.newKieBuilder(kfs);

			kieBuilder.buildAll();

			//session = kieServices.newKieContainer(kieServices.getRepository().getDefaultReleaseId()).newKieSession();
...
//			kContainer.
//			kContainer = kieServices.newKieContainer(null, kContainer.getClassLoader());
//			KieContainer kContainer = kieServices.newKieContainer(kieBuilder.getKieModule().getReleaseId());
			//KieContainer kContainer = kieServices.newKieContainer(kieServices.getRepository().getDefaultReleaseId());
			//KieContainer kContainer = KieServices.Factory.get().newKieContainer( kieBuilder.getKieModule().getReleaseId() );
			//kContainer = KieServices.Factory.get().newKieContainer(kContainer);
